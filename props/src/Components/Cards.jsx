...
const Cards = ({data,handleClick,cardindex}) => {
  ...
  return (
    <div className='w-60 bg-zinc-100 p-4 rounded-md flex gap-4 pb-10 relative'>
        ...
        <button onClick={()=>handleClick(cardindex)} className={`px-4 py-3 ${added===false?"bg-orange-600":"bg-teal-600"} whitespace-nowrap absolute bottom-0 left-1/2 -translate-x-[50%] translate-y-[50%] text-white text-xs rounded-full`}>{added===false?"Add to Favourities":"Added"}</button>
    </div>
  )
}
